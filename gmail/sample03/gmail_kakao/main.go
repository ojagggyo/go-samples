...
func run(ctx context.Context, path string, send, watch bool) error {
	...
	if errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("設定ファイル %q がありません。config.example.json をこのパスにコピーし、設定を確認してください（既定の場合: Copy-Item config.example.json config.json）。別の設定ファイルは -config で指定できます: %w", path, err)
	}
	if err != nil {
		return err
	}
