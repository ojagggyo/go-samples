...
func ReadWitnessCSV(filename string) ([]Witness, error) {
	...
	for {
		...
		case len(row) >= 9:
			// New format:
			// Name,Votes,RunningVersion,SigningKey,TotalMissed,
			// LastUpdate,LastBlockTime,Miss,SigningKeyChange
			w.LastBlockTime = row[6]
			w.Miss, _ = strconv.ParseInt(row[7], 10, 64)
			w.SigningKeyChange, _ = strconv.Atoi(row[8])

		case len(row) == 8:
			// Old compare format:
			// Name,Votes,RunningVersion,SigningKey,TotalMissed,
			// LastUpdate,Miss,SigningKeyChange
			w.Miss, _ = strconv.ParseInt(row[6], 10, 64)
			w.SigningKeyChange, _ = strconv.Atoi(row[7])

		case len(row) == 7:
			// New non-compare format:
			// Name,Votes,RunningVersion,SigningKey,TotalMissed,
			// LastUpdate,LastBlockTime
			w.LastBlockTime = row[6]
		}
